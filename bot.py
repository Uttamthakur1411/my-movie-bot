...
# --- 5. FIXED STREAMING LINK GENERATOR (Works on ALL devices) ---
def get_streaming_link(media_type, tmdb_id):
    """Multiple working streaming sources for all devices"""
    streaming_links = []
    
    # Primary working link (Mobile + Desktop)
    if media_type == 'movie':
        # Vidsrc.to - Works everywhere
        streaming_links.append(f"https://vidsrc.to/embed/movie/{tmdb_id}")
        # Alternative 1
        streaming_links.append(f"https://vidsrc.me/embed/movie/{tmdb_id}")
        # Alternative 2 - FlixHQ
        streaming_links.append(f"https://flixhq.to/embed/movie/{tmdb_id}")
    else:  # TV Series
        streaming_links.append(f"https://vidsrc.to/embed/tv/{tmdb_id}")
        streaming_links.append(f"https://vidsrc.me/embed/tv/{tmdb_id}")
    
    return streaming_links[0]  # Return primary working link

# --- 6. STRICT AUTH & SUBSCRIPTION CHECK ---
...
# --- 7. ADMIN PRO FEATURES (Broadcast, Stats & Add) ---
...
# --- 8. COMMANDS (Start, Refer, Watchlist, Continue) ---
...
async def movie_search(client, message):
    ...
    # FIXED STREAMING LINK - Works on ALL devices
    ...
    btns = [
        [
            ...
            InlineKeyboardButton("🎬 Trailer", url=f"https://www.youtube.com/results?search_query={urllib.parse.quote(title)}+trailer")
        ],
...
if __name__ == "__main__":
    print("🚀 PRO Bot is starting with FIXED streaming links...")
    keep_alive()
    bot_app.run()
